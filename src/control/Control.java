...
//				System.out.println(player.getCards().get(i).getcName());
	...
	private void stopInHouse(EcoPark b, PlayerModel player) {
		if (b.isEcoParkPurchasable()) {// 玩家房屋
	...
	private void upHouseLevel(EcoPark b, PlayerModel player) {
		if (b.canEcoParkUpLevel()) {
			...
			String name = b.getEcoHospitalName();
	...
	private void buyHouse(EcoPark b, PlayerModel player) {
		...
		int choose = JOptionPane.showConfirmDialog(
				null,
				"亲爱的:" + player.getName() + "\r\n" + "是否购买下这块地？\r\n"
						+ b.getEcoHospitalName() + "→" + b.getUpName() + "\r\n" + "价格："
						+ price + " 金币.");
...
//			System.out.println(srcPlayer.getCards().size() + "zhang");
	...
	private void useReduceLevelCard(Card card) {
		...
		if (ecoPark.getOwner() != null
				&& ecoPark.getOwner().equals(card.getOwner().getOtherPlayer())) {// 是对手的房屋
			if (ecoPark.getEcoParkLevel() > 0) { // 可以降级
				// 降级
	...
	private void useHaveCard(Card card) {
		...
		EcoPark ecoPark = this.building.getBuilding(
				card.getOwner().getY() / 60, card.getOwner().getX() / 60);
		if (ecoPark.getOwner() != null
				&& ecoPark.getOwner().equals(card.getOwner().getOtherPlayer())) {// 是对方的房屋
			Object[] options = { "确认使用", "重新选择" };
			int response = JOptionPane.showOptionDialog(null,
					"确认使用\"购地卡\"将此地收购？需要花费：" + ecoPark.getEcoParkAllPrice() + " 金币.",
					"卡片使用阶段.", JOptionPane.YES_OPTION,
					JOptionPane.PLAIN_MESSAGE, null, options, options[0]);
			if (response == 0) {
				if (card.getOwner().getCash() >= ecoPark.getEcoParkAllPrice()) {
					// 金币交换
					ecoPark.getOwner().setCash(
							ecoPark.getOwner().getCash()
									+ ecoPark.getEcoParkAllPrice());
					card.getOwner().setCash(
							card.getOwner().getCash() - ecoPark.getEcoParkAllPrice());
					ecoPark.setOwner(card.getOwner());
					// 增加文本提示
					this.textTip.showTextTip(card.getOwner(), card.getOwner()
	...
	private void useChangeCard(Card card) {
		EcoPark ecoPark = this.building.getBuilding(
				card.getOwner().getY() / 60, card.getOwner().getX() / 60);
		if (ecoPark.getOwner() != null
				&& ecoPark.getOwner().equals(card.getOwner().getOtherPlayer())) {// 是对手房屋
			Object[] options = { "确认使用", "重新选择" };
			int response = JOptionPane.showOptionDialog(null,
			...
			if (response == 0) {
				...
				for (EcoPark a : card.getOwner().getBuildings()) {
				...
				if (changeEcoPark != null) {
					changeEcoPark.setOwner(card.getOwner().getOtherPlayer());
					ecoPark.setOwner(card.getOwner());
					// 增加文本提示
					this.textTip.showTextTip(card.getOwner(), card.getOwner()
							.getName()
	...
	private void useAddLevelCard(Card card) {
		EcoPark ecoPark = this.building.getBuilding(
				card.getOwner().getY() / 60, card.getOwner().getX() / 60);
		if (ecoPark.getOwner() != null
				&& ecoPark.getOwner().equals(card.getOwner())) {// 是自己的房屋
			if (ecoPark.canEcoParkUpLevel()) { // 可升级
				// 升级
				ecoPark.setEcoParkLevel(ecoPark.getEcoParkLevel() + 1);
				// 增加文本提示
				this.textTip.showTextTip(card.getOwner(), card.getOwner()
